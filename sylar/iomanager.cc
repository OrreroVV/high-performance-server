...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    //如果events存在的话，则只需修改，否则添加
    int op = fd_ctx->events ? EPOLL_CTL_MOD : EPOLL_CTL_ADD;
    epoll_event epevent;

    //创建新的epollevent，设置边缘触发模式 (EPOLLET)，确定哪些类型：、之前已有的事件类型和新事件类型 event。
...
void IOManager::idle() {
    ...
    while(true) {
        ...
        do {
            ...
            //epoll_wait返回的是待处理的事件数, 返回到events中
